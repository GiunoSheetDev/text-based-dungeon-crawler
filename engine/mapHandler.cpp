...
void MapHandler::saveGridToJsonFile(const string filename) {
    picojson::array jsonGrid;

    for (const auto& row : grid) {
        picojson::array jsonRow;
        for (int cell : row) {
            jsonRow.push_back(picojson::value(static_cast<double>(cell)));
        }
        jsonGrid.push_back(picojson::value(jsonRow));
    }

    picojson::value jGrid(jsonGrid);

    ofstream file(filename);
    if (file.is_open()) {
        file << jGrid.serialize(true);  // pretty print JSON
    }
}
